...
# Generate a valid initial solution for Set Partitioning
def initial_solution(num_rows, num_columns, coverage):
    ...
    while uncovered_rows:
        ...
        if coverage[col] & uncovered_rows:
            selected_columns.add(col)
            uncovered_rows -= coverage[col]
...
def is_valid(solution, coverage, num_rows):
    covered_rows = set()
    for col in solution:
        if coverage[col] & covered_rows:  # No row should be covered twice
            return False
        covered_rows.update(coverage[col])
    return covered_rows == set(range(1, num_rows + 1))
