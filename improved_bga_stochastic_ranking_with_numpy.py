...
def genetic_algorithm_set_partition(constraint_matrix, column_costs, max_iterations=100, population_size=200, constraint_violation_penalty=10000, p=0.45, debug=True):
    ...
    for iteration in range(max_iterations):
        ...
        for child in new_offspring:
            ...
            population = stochastic_ranking(population, child, fitness, violation, fitness_values, violation_values, p)
            fitness_values, violation_values = evaluate_population(population, constraint_matrix, column_costs, constraint_violation_penalty)
        ...
        if debug:
            print(f'generation:{iteration}, violation:{best_violation}, cost min:{best_fitness},fitness mean:{np.mean(fitness_values)}, unfitness mean:{np.mean(violation_values)} ,cost max:{max(fitness_values)}, std:{round(np.std(fitness_values))}')
...
def ibgasr(file_path, debug=False):
